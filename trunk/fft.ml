...
let création_des_echantillons fichier=
  ...
  *)
  ...
  let _=for i=0 to paquets_d'echantillons-1 do
    let _=bigarray_courante:=Bigarray.Array1.create Bigarray.complex64 Bigarray.c_layout (echantillons_par_dt/nombre_de_canaux) in
    let _=for i=0 to echantillons_par_dt/nombre_de_canaux-1 do
...
let transformée_de_fourier fichier=
  ...
  let fft=Fftw2.create Fftw2.backward (echantillons_par_dt/nombre_de_canaux) in
