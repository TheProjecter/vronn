...
let propagation entree tab_poids f=
	...
	for i=0 to n-1 do
       tab_sorties.(i+1) (*a indice k*)<- apply f (multiply tab_poids.(i) (*W indice k*) tab_sorties.(i) (*a indice k-1*))
		done;
	tab_sorties
	;;
...
(* peut etre qun multiply irait aussi bien, sauf que la complexité, dont on se fout pas mal,
est meilleure quand meme dans celui la -> O(n) au lieu de O(n^3) *)

	let n=Array.length f_point_matrix in
	if Array.length f_point_matrix.(0) <> n 
		then raise Pas_carree
		else let res=Array.make_matrix n n 0. in
			for i=0 to n-1 do res.(i).(i) <- f_point_matrix.(i).(i) n_k.(i).(0) done;
			res
			;;
...
let sensib sortie_desiree tab_sorties f_point_matrix tab_poids=
	...
	let tmp= multiply (gradfpoint f_point_matrix tab_sorties.(m-1) (*dernière entree donc avant dernière sortie*) ) erreur in
...
let mAJ tab_poids tab_sensib tab_sorties eta=
	for i=0 to Array.length tab_poids -1 do
     tab_poids.(i) <- tab_poids.(i) -- (apply (fun x -> (0. -. eta) *. x) (multiply tab_sensib.(i) (transpo tab_sorties.(i+1))))
		done
	;;
